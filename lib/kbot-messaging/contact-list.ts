...
export function toKBotContactRows(input: {
  ...
  return input.contacts.map((contact) => {
    ...
    const matches = [byKey.get(subjectKeyForClient(contact.id)), phone ? byKey.get(phone) : undefined]
    ...
    if (!phone) {
      return { id: contact.id, name: contact.name, phone: contact.phone, state: 'NO_PHONE' as const }
    }
