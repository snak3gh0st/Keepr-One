...
function carrierDate(value: unknown): value is string {
...
export function parseFlexLifeQuoteSnapshot(value: unknown): FlexLifeQuoteSnapshotV1 | null {
  ...
  if (!snapshot || !exact(snapshot, ['schemaVersion', 'illustrationId', 'request']) ||
  ...
  if (!request || !exact(request, [
    ...
    'PremiumMode',
  ]) || !(ISSUE_STATES as readonly unknown[]).includes(request.IssueState) ||
    !safeName(request.FirstName) || !safeName(request.LastName) || !carrierDate(request.DateOfBirth) ||
  ...
  return { schemaVersion: 1, illustrationId: snapshot.illustrationId, request: request as RapidSolveRequest }
}
...
export async function sha256FlexLifeQuoteSnapshot(value: unknown): Promise<string> {
