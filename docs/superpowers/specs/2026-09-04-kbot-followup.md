...
AI initially supports Evolution only. Meta Cloud free-text initiation is blocked pending approved-template support. Live Evolution connection and sender identity are verified before generation and again before dispatch. Automated messages use Evolution's authenticated internal API so the job receives the exact WhatsApp message id; Evolution mirrors that outbound message into the agent's Chatwoot conversation. Reconciliation requires that exact id, recipient and sender identity before accepting provider ACKs. Manual messages continue through Chatwoot. No browser or desktop session performs the send.
