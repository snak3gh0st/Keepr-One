...
describe('NationalPremiumEvolution', () => {
  it('renders source caveat, accessible exact values and URL-backed controls', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Prêmio anual previsto' })).toBeTruthy()
    expect(screen.getByRole('region', { name: 'Prêmio anual previsto' })).toHaveClass('bg-rail-strong', 'text-paper')
    ...
    expect(screen.getByRole('img')).toHaveAccessibleName('Prêmio anual previsto em dólares por mês de emissão')
