...
export default async function AgentDashboard({
  ...
  // The portfolio block below, the National Life queues and the calendar read
  // disjoint tables and never consume each other's results, yet they used to run
  // as three serial stages. Starting them here lets them overlap; each keeps its
  // own catch so one failing source still degrades on its own.
  const nationalQueuesPromise = canUsePolicies
    ? loadNationalPolicyQueues(prisma, scope).catch((error) => {
      console.error('National policy queue query error', error)
      return null
    })
    : Promise.resolve(null)

  const calendarPromise = canUseCalendar
    ? (async () => {
      try {
        ...
        const sources = mappedCalendar.calendars
        const calendarById = new Map(sources.map((calendar) => [calendar.id, calendar]))
        ...
        const today = todayCalendarResult.upcoming.map((event) => mapDomainCalendarEventToUi(event, {
        ...
        const upcoming = upcomingCalendarResult.map((event) => mapDomainCalendarEventToUi(event, {
          timeZone: user?.timeZone ?? 'America/New_York',
          case: event.caseId ? meetingCaseById.get(event.caseId) ?? null : null,
          canWrite: calendarById.get(event.calendar.id)?.canWrite ?? false,
        }))
        return { connection: mappedCalendar.connection, sources, today, upcoming }
      } catch (error) {
  ...
  const queueResult = await nationalQueuesPromise
  if (queueResult?.verified) nationalQueueCounts = queueResult.counts

  const calendarResult = await calendarPromise
  if (calendarResult) {
    calendarConnection = calendarResult.connection
    calendarSources = calendarResult.sources
    todayMeetings = calendarResult.today
    upcomingMeetings = calendarResult.upcoming
  }
