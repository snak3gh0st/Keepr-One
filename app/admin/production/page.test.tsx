...
type ProductionRow = { commissionTotal: number }
...
vi.mock('@/components/ContextPanel', () => ({ ContextPanel: ({ children }: PropsWithChildren) => <div>{children}</div> }))
vi.mock('./ProductionTable', () => ({ ProductionTable: ({ rows }: { rows: readonly ProductionRow[] }) => <div>{rows[0]?.commissionTotal}</div> }))
afterEach(cleanup)
